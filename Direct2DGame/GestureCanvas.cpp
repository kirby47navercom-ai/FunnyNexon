...
std::optional<GestureResult> GestureCanvas::Update(const InputState& input, float deltaTime) {
    ...
    if (!m_loaded || !m_recognizer || !m_recognizer->IsLoaded()) {
        return std::nullopt;
    }
...
void GestureCanvas::Draw(Renderer& renderer) const {
    ...
    const float checkY = CanvasHeight - 43.0f - 90.0f * m_openAmount;

    if (m_openAmount <= 0.0f) {
        renderer.DrawImageClip(L"Assets\\Canvas\\2.png", 0.0f, 0.0f, 825.0f, 216.0f,
            CanvasWidth * 0.5f, checkY, 825.0f * 0.4f, 216.0f * 0.4f, false, 0.95f);
        return;
    }
    ...
    if (IsReadyToDraw()) {
        const std::wstring status = m_loaded ? L"그림을 그리고 마우스를 떼세요." : L"제스처를 준비 중이야...";
        renderer.DrawTextPico(status, 18.0f, CanvasHeight - 86.0f, 440.0f, 34.0f, 26.0f,
